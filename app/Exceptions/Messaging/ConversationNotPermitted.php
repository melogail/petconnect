 ...
 *
 * Today the rule is `users.is_active`: a deactivated account is not messaged. It
 * is stated once, in App\Models\User::acceptsMessagesFrom(), and every seam asks
 * that method. A recipient-side block list and per-recipient message settings
 * are not built (they are their own vertical, with their own table and their own
 * UI), but when they arrive they are new checks in that one method rather than a
 * new decision in a step, a controller or a policy.
 *
 * The message is deliberately the same whatever the reason. "This person has
 * blocked you" and "this account is deactivated" are different facts about
 * another user, and a stranger who can tell them apart by probing has learned
 * something about a person who did not choose to tell them.
 *
